# 🚀 Быстрый старт

## Шаг 1: Клонирование и установка

```bash
# Клонировать репозиторий (если еще не клонирован)
# git clone <repo-url>
# cd consolidator-premium

# Установить зависимости
npm install
```

## Шаг 2: Настройка базы данных

Создайте файл `.env` в корне проекта:

```bash
cat > .env << 'EOF'
DATABASE_URL="file:./prisma/dev.db"
NODE_ENV=development
EOF
```

Или создайте вручную с содержимым:

```env
DATABASE_URL="file:./prisma/dev.db"
NODE_ENV=development
```

## Шаг 3: Инициализация Prisma

```bash
# Генерация Prisma Client
npm run prisma:generate

# Создание базы данных и применение миграций
npm run prisma:migrate

# (Опционально) Открыть Prisma Studio для просмотра БД
npm run prisma:studio
```

## Шаг 4: Запуск проекта

```bash
# Запуск в режиме разработки
npm run dev
```

Откройте браузер:
- **Frontend**: http://localhost:3000/catalog
- **API Docs**: см. [server/README.md](./server/README.md)

## Шаг 5: Тестирование API

### Вариант 1: Использовать готовые примеры

```bash
# Сделать файл исполняемым
chmod +x examples/api-examples.sh

# Запустить все примеры
./examples/api-examples.sh
```

### Вариант 2: Ручные запросы

```bash
# Получить контрольные точки
curl http://localhost:3000/api/events/ev_test_early/control-points

# Имитация платежа (Visa)
curl -X POST http://localhost:3000/api/payments/simulate \
  -H "Content-Type: application/json" \
  -d '{
    "eventId": "ev_test_early",
    "cardNumber": "4532015112830366",
    "expiry": "12/25",
    "cvc": "123",
    "amount": 1000
  }'
```

## Шаг 6: Запуск тестов

```bash
# Все тесты
npm test

# С UI интерфейсом
npm run test:ui

# В watch режиме
npm test -- --watch
```

## 🎯 Тестовые данные

### Продюсеры (для создания мероприятий)
- **прод1** / пар1
- **прод2** / пар2

### Тестовые карты

| Тип | Номер | CVV | Срок |
|-----|-------|-----|------|
| Visa | `4532015112830366` | 123 | 12/25 |
| MasterCard | `5425233430109903` | 456 | 03/26 |
| Mir | `2200000000000004` | 789 | 06/27 |

### Невалидные карты (для тестирования ошибок)
- `1234567890123456` - не проходит Luhn
- `0000000000000000` - не проходит Luhn

## 📁 Основные файлы

```
consolidator-premium/
├── server/
│   ├── api/                    # API эндпоинты
│   │   ├── events/
│   │   │   └── [id]/
│   │   │       ├── control-points.get.ts
│   │   │       └── control-point.put.ts
│   │   └── payments/
│   │       └── simulate.post.ts
│   ├── utils/                  # Утилиты
│   │   ├── getNotices.ts      # Тексты извещений
│   │   ├── cardValidator.ts   # Валидация карт
│   │   └── prisma.ts          # Prisma client
│   └── README.md              # Полная документация API
├── prisma/
│   ├── schema.prisma          # Схема БД
│   └── migrations/            # Миграции
├── tests/                     # Тесты
│   ├── getNotices.test.ts
│   └── cardValidator.test.ts
└── examples/
    └── api-examples.sh        # Примеры запросов

```

## 🔧 Полезные команды

```bash
# Разработка
npm run dev                    # Запуск dev сервера
npm test                       # Запуск тестов
npm run test:ui               # Тесты с UI

# Prisma
npm run prisma:generate       # Генерация Prisma Client
npm run prisma:migrate        # Применение миграций
npm run prisma:studio         # Открыть Prisma Studio

# Production
npm run build                 # Сборка для production
npm run start                 # Запуск production сервера
```

## ❓ Часто задаваемые вопросы

### Ошибка: "Cannot find module '@prisma/client'"

```bash
npm run prisma:generate
```

### Ошибка: "Environment variable not found: DATABASE_URL"

Создайте файл `.env` с содержимым:
```env
DATABASE_URL="file:./prisma/dev.db"
```

### Как сбросить базу данных?

```bash
rm prisma/dev.db
npm run prisma:migrate
```

### Как добавить тестовые данные?

База данных будет пустой после миграций. Вы можете:
1. Использовать существующие mock-данные из `/public/mock/events.json`
2. Создавать события через frontend (/create-event)
3. Добавить seed-скрипт для Prisma

## 📚 Дополнительная информация

- Полная документация API: [server/README.md](./server/README.md)
- Основной README: [README.md](./README.md)

## 🆘 Поддержка

Если возникли проблемы:
1. Убедитесь, что все зависимости установлены: `npm install`
2. Проверьте, что файл `.env` создан
3. Убедитесь, что Prisma Client сгенерирован: `npm run prisma:generate`
4. Проверьте логи: `npm run dev` должен показать подробные ошибки

---

**Готово! 🎉** Теперь вы можете работать с виртуальной платежной системой!

