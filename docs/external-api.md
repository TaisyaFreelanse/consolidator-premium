...
- **Опубликовано (`published`)**: После публикации мероприятие становится доступно всем пользователям платформы. Публикация происходит автоматически или через модерацию.
...
**Важно:** Статус мероприятия зависит от настроек сайта в белом списке. Сайты без модерации создают события сразу в статусе `published`, сайты с модерацией - в статусе `draft` до одобрения модератором.
...
### ~~POST `/api/external/events/publish`~~ (Удален)
