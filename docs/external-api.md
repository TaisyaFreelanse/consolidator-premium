...
`author` | string | да | Идентификатор автора из справочника ядра.
...
`endAt` | ISO datetime | рекомендуется | Контрольная точка `ti50`, окончание мероприятия.
...
`producerName` | string | рекомендуется | Отображаемое имя продюсера.
`description` | string | нет | Краткое описание мероприятия.
...
1. **Обязательные поля**: `title`, `author`, `location`, `seatLimit`, `pricePerSeat`, `startApplicationsAt`, `endApplicationsAt`, `startContractsAt`, `startAt`, `timezone`, `producerCode`.
...
6. **Формат дат**:
   - Принимаем строку в ISO 8601 (`YYYY-MM-DDTHH:mm:ssZ`).
