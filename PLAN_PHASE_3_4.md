...
### Задачи:
- [ ] Подготовить полную документацию по внешнему API
- [ ] Описать все эндпоинты для внешних запросов
- [ ] Указать параметры запросов и форматы ответов
- [ ] Добавить примеры использования API
- [ ] Разместить документацию на странице "Настройки/регистр"
...
**Задачи:**
- [ ] Добавить кнопку запроса мониторинга на странице "Отчет платформы" (`/demo/platform-report`)
- [ ] Добавить кнопку "Отчет платформы" в верхнее меню (DemoNavigation.vue), справа от кнопки "Загрузить на платформу"
- [ ] Реализовать проверку наступления Ти20 перед запросом
- [ ] Использовать существующий API платформы для получения данных мониторинга (или создать эндпоинт на платформе, если его нет)
- [ ] Реализовать запрос с демо-сайта к API платформы для получения данных мониторинга
- [ ] Обработать ответ от платформы и отобразить данные на демо-сайте

**Данные по каждому заявителю:**
- [ ] Признак вхождения в лимит (или причина невхождения)
- [ ] Список всех платежей заявителя (реквизиты от ЮKassa)
...
- [ ] Реализовать расчет "Возврат сверхлимитчикам" (сумма всех платежей от заявителей вне лимита)
- [ ] Реализовать расчет "Профицит к распределению" (ПРОФИЦИТ - Возврат сверхлимитчикам)
- [ ] Добавить отображение всех пяти чисел в компоненте `EventStatus.vue` (справа от "Профицит")
- [ ] Добавить отображение всех чисел в таблице мониторинга (`components/MonitoringTable.vue`)
- [ ] Добавить отображение всех чисел в PDF-отчетах (для каждого заявителя)
- [ ] Реализовать замену секретного кода на логин только для авторизованного пользователя (в таблице мониторинга)
- [ ] Для остальных заявителей показывать секретный код (не заменять на логин)
...
**Задачи:**
- [ ] Выбрать библиотеку для генерации PDF (jsPDF, pdfkit, puppeteer)
- [ ] Выбрать библиотеку для создания ZIP-архивов (JSZip)
- [ ] Создать шаблон PDF с информацией мониторинга
- [ ] Добавить таблицу платежей в PDF
- [ ] Реализовать генерацию отдельного PDF для каждого заявителя
- [ ] Реализовать формирование ZIP-архива с правильной структурой имен
- [ ] Реализовать скачивание ZIP-архива (браузер предложит выбрать папку для сохранения)
...
**Задачи:**
- [ ] Переименовать страницу `/demo/api-register` → `/demo/settings` (или обновить заголовок)
- [ ] Обновить навигацию (DemoNavigation.vue)
- [ ] Добавить раздел управления API-ключом
- [ ] Добавить раздел с документацией API
- [ ] Обновить все ссылки на старую страницу
